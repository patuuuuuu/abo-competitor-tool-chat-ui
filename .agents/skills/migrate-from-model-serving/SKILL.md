...
**Before doing anything else, ask the user three questions.** Use the `AskUserQuestion` tool to collect all answers at once so the user is only prompted once, then Codex can execute the rest of the migration autonomously.
...
Copy all scaffold files from the current working directory into a new directory named `<app-name>/`. Exclude instruction files (`AGENTS.md`, `AGENTS.md`), hidden directories (`.Codex/`, `.git/`), and any migration artifacts (e.g., `original_mlflow_model/`, `.migration-venv/`). Do NOT search for or copy scaffold files from other directories or templates — everything you need is right here.
...
- **LLM endpoint name** (e.g., `databricks-Codex-sonnet-4-5`)
- **System prompt**
- **Tool definitions**
- **Any custom logic**
...
resources:
  ...
  databricks:
    ...
    - name: lakebase
    serving_endpoint:
    - name: databricks-Codex-sonnet-4-5
```
